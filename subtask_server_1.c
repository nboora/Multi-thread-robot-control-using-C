...
int send_msg(char msg[])
{
    ...
    //Send the message to Main Server
...
{
    printf("starting client thread function\n");
    ...
    char test_type[]="Enter your selection:\ns1_task1\n";
    ...
    {
        printf("client_thread function Send Failed. Error!!!!!\n");
        return 0;
    }
    ...
    //Receive the message from the client
        ...
        {
            printf("client_thread function Receive Failed. Error!!!!!\n");
            return 0;
        }
        printf("client_thread function Client Sock: %i\n",client_sock);
        printf("client_thread function Client Message: %s\n",client_message);
        memset(server_message,'\0',sizeof(server_message));                //Cleaning the Buffers
        memset(port_buffer,'\0',sizeof(port_buffer));
        
        // checking for the task input and verifying from the client
        if(strcmp(client_message,"s1_task1")==0)
        {
            ...
            strcat(server_message,",kinematics,");
            strcat(server_message,client_message);
            strcat(server_message," \n");
            strcat(server_message, "**********************************************************************\n\n");
            strcat(server_message, "The robot started to perform the kinematics task from the sub server-1\n\n");
            strcat(server_message, "**********************************************************************\n\n");
        }
        else
...
{   
    int port=atoi(my_port);                             // defining port as int by using atoi.
    int socket_desc, client_sock, client_size;        
        ...
        socket_desc = socket(AF_INET, SOCK_STREAM, 0);  // Creating a socket that returns a socket descriptor
        if(socket_desc < 0)
        {
            printf("Could Not Create Socket. Error!!!!!\n");
            return -1;
        ...
        while(1)       // infinite loop
        {
            ...
            else
            {
                ...
                strcpy(info->ip,inet_ntoa(client_addr.sin_addr)); // converts the host address, given in network byte order, to a string and copies it.
                info->port=ntohs(client_addr.sin_port);           // converts the netshort from network byte order to host byte order.
                info->socket=client_sock;
                info->loc=info;
                int ret = pthread_create(&thread, NULL, client_thread, (void *)info); // creating a pthread
                if (ret!=0)
                {
...
int main(int argc, char** argv) {
    char msg[100];
    char my_test_name[]="kinematics";   // defining the name of the run test
